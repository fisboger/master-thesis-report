...
As it can be seen, the message \emph{"Reassembly failure: packets do not add up correctly"} is displayed which is an event generated by \mintinline{c}{Ipv6pReassembleDatagram}, which allows us to detect exploitation attempts of CVE-2021-24086. However, the \gls{ETW} event are only written on patched machines, so it is uneffective for detection exploitation attempts on unpatched machines.
\\
\\
\todo{Write about how we can only detect post-patch, but that this also proves that ETW can be used, if only the "right" events are written}

% Only detect post patch
% Because now we know this is an unintended state
% Showcase that ETW can be used for detection of this type, but it requires more "generic" events.
% Generic event could be more debug related, such as "Reassembling packet with x bytes"
